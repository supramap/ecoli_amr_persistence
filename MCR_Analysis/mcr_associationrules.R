...
############################3
## Use Current Rules against Validation Set
...
## Loop through current rules against validation dataset (with the pattern {*|-mcr} => {mcr})
...
for (i in 1:nrow(orig_mcr_rules_df)){
  ## Get rule size lhs/rhs set to find
  size <- orig_mcr_rules_df$size[i]
